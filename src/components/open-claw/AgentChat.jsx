...
import { panelStyle, inputStyle, btnPrimaryStyle } from "./styles";
...
export default function AgentChat({ agents = [], onSendMessage }) {
  ...
  useEffect(() => {
    if (agents.length > 0 && !active) setActive(agents[0]);
  }, [agents, active]);
  ...
  const send = () => {
    ...
    if (onSendMessage) {
      Promise.resolve(onSendMessage(active.id, text)).then(response => {
        setMessages(p => ({ ...p, [active.id]: [...(p[active.id] || []), { from: "agent", text: response }] }));
        setTyping(false);
      });
    } else {
      setTimeout(() => {
        setMessages(p => ({ ...p, [active.id]: [...(p[active.id] || []), { from: "agent", text: `[${active.id}] Message received.` }] }));
        setTyping(false);
